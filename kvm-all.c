                                         ...
                                         unsigned long mem_size)
{
    ...
    for (i = 0; i < len; i++) {
		...
		tlc_cpu_physical_memory_set_dirty(ram_addr); // TLC
...
// TLC profiling
	    if(unlikely(mthread)){
    	    	//(tlc_profile[cpu_index].basic_io_cnt)++; 
    	    	(tlc_profile[cpu_index].total_io_cnt)++; 
	    }
...
// TLC profiling
	    if(unlikely(mthread)){
   		//(tlc_profile[cpu_index].mmio_cnt)++; 
    	    	(tlc_profile[cpu_index].total_io_cnt)++; 
	    }
