...
{
// TLC
//error_report(" n->nic->nc.peer checking");
    if (!n->nic->nc.peer)
        return 0;

//error_report(" n->nic->nc.peer = exist");
//error_report(" n->nic->nc.peer->info->type = %d != %d\n", n->nic->nc.peer->info->type, NET_CLIENT_TYPE_TAP);
    if (n->nic->nc.peer->info->type != NET_CLIENT_TYPE_TAP)
        return 0;

    n->has_vnet_hdr = tap_has_vnet_hdr(n->nic->nc.peer);

//error_report(" n->has_vnet_hdr = %d\n", n->has_vnet_hdr);
...
static int virtio_net_load(QEMUFile *f, void *opaque, int version_id)
{
    ...
    if (version_id >= 7) {
        if (qemu_get_be32(f) && !peer_has_vnet_hdr(n)) {
            error_report("virtio-net: saved image requires vnet_hdr=on check");
            return -1;
        }
